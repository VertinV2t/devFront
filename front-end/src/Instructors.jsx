...
export default function Instructors(){
    ...
    function display() {
        return data.filter((info) => 
            search == '' ? true:info.name.includes(search)
        )
    }
    ...
    const addNew = async() =>{
        const data = await axios.post('http://localhost:3000/instructors', {
                        name: inputFN,
                        bio: inputBIO
                    })
        if(data){
                    ...
                    <div className='grid grid-cols-2 items-start border overflow-y-scroll'>
                        {
                            display().slice(page[0],page[1]).map((item, index) => (
                                <Link key={index} className='text-white' to={'/instructors/' + item.id}>
                                    <p className='text-2xl p-4 border'>{item.name}</p>
                                </Link>
                            ))
                        }
                    </div>
